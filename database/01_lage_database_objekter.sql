...
-- --------------------------------------------------------------------------------------------------------------------------------------------------------------
-- --------------------------------------------------------------------------------------------------------------------------------------------------------------
-- -- If not using docker-compose, use this 3 commands to create the database, the user and grant the access rights
...
(
	...
	, T1.WORDID
	, COUNT(*) AS COUNT
	, MAX(T2.ID) AS BRONZE_PRICE_ID
	 FROM WORD_LOG T1
	 LEFT JOIN PRICE_POINT T2 ON T1.ID = T2.ID
	AND T1.PID = T2.PID
	AND T1.WORDID = T2.WORDID
	AND T2.WORDRANK = 5
	 WHERE T1.endmillisecond-T1.startmillisecond BETWEEN 0 AND 4000
	 GROUP BY
	 T1.PID
	, T1.WORDID
...
, CASE 
	...
	ELSE T2.COUNT
	END AS BRONZE_STARS
, T2.BRONZE_PRICE_ID AS MAX_BRONZE_WORD_LOG_ID -- DETTE ER ETT MISSLEDENDE NAVN, MEN ENDRER IKKE PGA VIEW MED AVHENGIGHET HIT
, T3.ENDTIME
FROM PLAYER_WORD T1
LEFT JOIN AGG_DATA T2 ON T1.PID = T2.PID AND T1.WORDID = T2.WORDID
LEFT JOIN WORD_LOG T3 ON T2.BRONZE_PRICE_ID = T3.ID;
...
(	SELECT T1.ID
	...
	FROM WORD_LOG T1 
	WHERE 1=1
	AND T1.endmillisecond-T1.startmillisecond < 2501 
)
,AGG_DATA AS
(
	SELECT 
...
, CASE 
	...
	ELSE T2.COUNT
	END AS SILVER_STARS
, T2.SILVER_PRICE_ID AS MAX_SILVER_WORD_LOG_ID  -- DETTE ER ETT MISSLEDENDE NAVN, MEN ENDRER IKKE PGA VIEW MED AVHENGIGHET HIT	
, T3.ENDTIME
FROM PLAYER_WORD T1
LEFT JOIN AGG_DATA T2 ON T1.PID = T2.PID AND T1.WORDID = T2.WORDID
LEFT JOIN WORD_LOG T3 ON T2.SILVER_PRICE_ID = T3.ID;
...
(
	SELECT PID, WORDID, MIN(ID) AS ID FROM AGG_DATA
	WHERE lag0 < 2501 
	AND lag1 < 2501 
	AND lag2 < 2501 
	AND lag3 < 2501 
...
, T4.ID AS MAX_GOLD_WORD_LOG_ID
, T3.ENDTIME
FROM PLAYER_WORD T1
LEFT JOIN AGG_DATA T2 ON T1.PID = T2.PID AND T1.WORDID = T2.WORDID AND last_word = 1
LEFT JOIN PRICE_POINT T4 ON T1.PID = T4.PID AND T1.WORDID =T4.WORDID
LEFT JOIN WORD_LOG T3 ON T4.ID = T3.ID;
